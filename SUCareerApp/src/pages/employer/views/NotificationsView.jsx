        ...
        {notifications.map(note => (
          <div key={note.id} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: '#94a3b8' }}>{note.time}</div>
            </div>
            {!note.read && (
              <button onClick={() => handleDismiss(note.id)} style={{ background: 'none', border: 'none', color: '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: '4px 8px' }}>
                Dismiss
              </button>
            )}
          </div>
